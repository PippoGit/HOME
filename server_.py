...
from home_module import miner, db, feed, utility, rss
...
db = db.dbconnector.DBConnector(**config['db'])
feed_parser = rss.Parser(config['feeds'])
                          
# loading models
print("\nloading the models...") 
news_classifier = miner.classification.load_news_classifier()
likability_predictor = miner.classification.load_likability_predictor()

# building the newsfeed... 
print("\nparsing the news...") 
newsfeed = feed.NewsFeed(news_classifier, likability_predictor)
feed_parser.parse()
